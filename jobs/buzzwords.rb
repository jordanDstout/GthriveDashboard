...
list_of_glocations=['54','212','204']
  ...
  for x in list_of_glocations do
    #4 lines used to get and parse json
  ...
  for key in farms
    ...
    #for each array within the larger array in farms (two items in each)
      for x in key1
        toFinal=x[0]
...
#send_event('farm',{items:[ status:'danger',label:"one", value: 'two']})
